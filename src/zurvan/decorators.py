"""
When building AI agents, one of the most challenging aspects
is maintaining accurate documentation about the tools our agent
can use. Every time we modify a tool's parameters or behavior,
we need to update its description and parameter schema.
If these get out of sync, our agent might try to use tools incorrectly,
leading to failures that can be hard to debug.

Benefits of having the decorator:
1. Single Source of Truth: The function itself becomes the authoritative source for all tool information. The docstring describes what it does, the type hints define its parameters,
and the implementation shows how it works.
